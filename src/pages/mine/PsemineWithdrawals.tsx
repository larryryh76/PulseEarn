      ...
      <div className="space-y-6">
        ...
        <div className="bg-[#12121A] border border-white/10 rounded-2xl p-6 relative overflow-hidden">
          ...
          <h1 className="text-2xl font-extrabold text-white tracking-tight flex items-center gap-2">
            ...
            <span>Mining Output Payouts</span>
          </h1>
          <p className="text-xs text-zinc-400 mt-1 max-w-xl">
            Request output settlement to your verified BEP-20 BNB Smart Chain wallet address.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 bg-[#12121A] border border-white/10 rounded-2xl p-5 space-y-4">
            <h3 className="text-xs font-bold text-white uppercase tracking-wider">Request Payout</h3>

            <form onSubmit={handleWithdrawalSubmit} className="space-y-3">
              <div>
                <label className="text-[10px] font-bold uppercase text-zinc-400 block mb-1">Amount (GBP)</label>
                <input
                  type="number"
                  step="0.01"
                  min="10"
              ...
              <div>
                <label className="text-[10px] font-bold uppercase text-zinc-400 block mb-1">BEP-20 Payout Address</label>
                <input
                  type="text"
                  placeholder="0x..."
                  value={payoutAddress}
              ...
              <div className="p-3 bg-white/5 rounded-xl border border-white/5 text-[11px] text-zinc-400 space-y-1">
                <p className="flex items-center gap-1 text-emerald-400 font-bold">
                  <ShieldCheck size={14} /> Settlement Security Check
                </p>
                <p className="text-[10px] text-zinc-400">Withdrawals undergo administrative review before execution on the BNB Smart Chain.</p>
              </div>

              <button
                type="submit"
                disabled={submitting}
                className="w-full py-2.5 bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold rounded-xl text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-[0_0_15px_rgba(16,185,129,0.3)] disabled:opacity-50"
              >
          ...
          <div className="md:col-span-2 bg-[#12121A] border border-white/10 rounded-2xl p-5 space-y-4">
            <h3 className="text-xs font-bold text-white uppercase tracking-wider">Payout History</h3>

            {loading ? (
              <div className="text-center text-xs text-zinc-500 py-6">Loading payout records...</div>
            ) : fetchError ? (
              ...
              <div className="text-center text-xs text-zinc-500 py-6">No withdrawal requests recorded yet.</div>
            ) : (
