...
export const PSEmineApp: React.FC = () => {
  ...
  const navItems = [{ href: '/mine/dashboard', label: 'Home', icon: Home }, { href: '/mine/tools', label: 'Tools', icon: Package }, { href: '/mine/activity', label: 'Activity', icon: Clock3 }, { href: '/mine/guide', label: 'Guide', icon: BookOpen }, { href: '/mine/me', label: 'Me', icon: UserRound }]
  return <div className="psemine-app-page">
    <nav className="psemine-app-nav" aria-label="PSEmine workspace"><Link to="/mine/dashboard" className="psemine-app-nav-brand">PSEmine</Link><div className="psemine-app-nav-links">{navItems.map(({ href, label, icon: Icon }) => <Link key={href} to={href} className={location.pathname === href ? 'is-active' : ''}><Icon />{label}</Link>)}<button onClick={async () => { await logout(); navigate('/mine') }} aria-label="Log out"><LogOut />Log out</button></div></nav>
    ...
    <main className="psemine-shell psemine-app-main">
      ...
      <section className="psemine-app-section"><div className="psemine-app-section-head"><div><span className="psemine-eyebrow">Wallets</span><h2>Settlement destination</h2></div><WalletCards size={22} /></div>{snapshot?.wallets?.length ? snapshot.wallets.map(wallet => <div className="psemine-wallet-row" key={`${wallet.role}-${wallet.address}`}><span>{wallet.role === 'payout' ? 'Payout wallet' : 'Connected wallet'}</span><strong>{wallet.address}</strong><small>{wallet.network} · {wallet.status}</small></div>) : <div className="psemine-empty-state">No settlement wallet is connected yet. Connect one from the Wallet page before purchasing tools or requesting payout.<a href="/mine/wallet">Open wallet settings <ArrowRight size={16} /></a></div>}</section>
    </main>
  </div>
}
