...
const Profile: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'IDENTITY' | 'ACHIEVEMENTS' | 'ACTIVITY'>('IDENTITY');
    ...
    <DashboardLayout>
      <div className="max-w-5xl mx-auto space-y-12 pb-24 animate-in">
        ...
        <section className="relative glass-panel rounded-[3rem] overflow-hidden border-white/10 shadow-2xl">
           ...
           <div className="relative pt-20 px-10 pb-12 flex flex-col md:flex-row items-center md:items-end justify-between gap-10">
              <div className="flex flex-col md:flex-row items-center md:items-center gap-8">
                 <div className="relative group">
                    <div className="w-40 h-40 rounded-[3rem] bg-surface border-4 border-surface overflow-hidden shadow-2xl relative z-10 group-hover:scale-[1.02] transition-transform duration-500">
                 ...
                 <div className="space-y-3 text-center md:text-left">
                    <div className="flex flex-col md:flex-row items-center gap-3">
                       ...
                       <div className="px-3 py-1 rounded-full bg-primary/10 border border-primary/20 flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-bold text-primary uppercase tracking-widest">LVL {userData.level} Agent</span>
                       </div>
                    </div>
                    <div className="flex items-center gap-4 text-white/40 text-sm font-medium justify-center md:justify-start">
                       ...
                       <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono text-xs uppercase pr-4">ID: {userData.uid.substring(0, 12)}</span>
                       </div>
                    </div>
                 </div>
              </div>
           ...
           {/* Tab Selection */}
           <div className="px-10 pb-8 flex items-center gap-10 border-t border-white/5 pt-8">
              {(['IDENTITY', 'ACHIEVEMENTS', 'ACTIVITY'] as const).map(tab => (
                 <button
                   key={tab}
           ...
           <div className="lg:col-span-8 space-y-10">
              ...
              {activeTab === 'ACHIEVEMENTS' && (
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                       { title: 'Genesis Agent', desc: 'Authorized during initial platform deployment.', icon: Trophy, unlocked: true },
                       { title: 'Market Oracle', desc: 'Predictive accuracy exceeding 75% on signals.', icon: Star, unlocked: true },
                       { title: 'Pulse Titan', desc: 'Maintain a balance exceeding 1,000,000 PTS.', icon: Zap, unlocked: false },
                       { title: 'Network Commander', desc: 'Onboard 100+ verified ecosystem participants.', icon: Share2, unlocked: false },
                    ].map((ach, i) => (
              ...
              <div className="glass-panel p-8 rounded-[2.5rem] space-y-6 bg-primary/[0.02] border-primary/10">
                 ...
                 <div className="p-5 bg-white/5 rounded-3xl border border-white/5 space-y-3">
                    <p className="text-sm font-bold leading-relaxed">
                       Achieve LVL {userData.level + 1} to unlock premium high-yield missions and prioritized audit cycles.
                    </p>
