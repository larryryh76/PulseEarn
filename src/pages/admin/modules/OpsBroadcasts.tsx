    ...
    <div className="space-y-12">
       <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="space-y-2">
             <div className="flex items-center gap-3 text-primary">
                <Bell size={20} />
                <h1 className="text-3xl font-bold tracking-tight uppercase italic text-white">Broadcast Matrix</h1>
             </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div
              ...
              className="bg-[#0A0A0F] border border-white/5 p-10 rounded-[2.5rem] hover:border-primary/20 transition-all cursor-pointer group shadow-2xl relative overflow-hidden"
            >
               ...
               <h3 className="text-lg font-bold text-white uppercase tracking-tight italic mb-3 group-hover:text-primary transition-colors">{tpl.title}</h3>
       ...
       <div className="space-y-8 pt-8 border-t border-white/5">
          <div className="flex items-center justify-between px-1">
             <h2 className="text-sm font-black uppercase tracking-[0.3em] text-white/20 flex items-center gap-3">
                <Globe size={18} className="text-indigo-400" />
                Transmission Stream
             </h2>
             <span className="text-[10px] font-mono text-white/10 uppercase tracking-widest">Global Broadcast Sync Active</span>
          </div>

          <div className="py-40 text-center bg-[#0A0A0F] border border-dashed border-white/10 rounded-[3rem] shadow-inner opacity-40 group hover:opacity-100 transition-opacity">
             <div className="w-20 h-20 rounded-full border border-dashed border-white/10 mx-auto flex items-center justify-center mb-6 group-hover:border-primary/20 transition-all">
                <Bell size={40} className="text-white/5 group-hover:text-primary/20 transition-all duration-700" />
             </div>
             <h3 className="text-sm font-bold uppercase tracking-widest text-text-tertiary mb-2">No active broadcasts</h3>
             <p className="text-[10px] font-mono text-white/10 uppercase tracking-widest">All communication channels currently silent</p>
          </div>
       </div>
          ...
          {isModalOpen && (
             <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-xl p-6">
                <motion.div
                   ...
                   className="relative w-full max-w-xl bg-[#0A0A0F] border border-white/10 rounded-[3rem] shadow-[0_0_100px_rgba(0,0,0,1)] overflow-hidden"
                >
                   <div className="p-10 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
                      <div className="flex items-center gap-6">
                         ...
                         <div>
                            ...
                            <p className="text-white/40 text-[10px] font-black uppercase tracking-widest leading-none">Authorized Platform Communication Vector</p>
                         </div>
                      </div>
                      <button onClick={() => setIsModalOpen(false)} className="w-10 h-10 hover:bg-white/5 rounded-xl transition-all flex items-center justify-center text-text-tertiary"><X size={24} /></button>
                   </div>

                   <form onSubmit={handleBroadcast} className="p-10 space-y-8">
                      <div className="space-y-2.5">
                         <label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 ml-1">Signal Title</label>
                         <input
                           required
                      ...
                      <div className="space-y-2.5">
                         <label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 ml-1">Transmission Data</label>
                         <textarea
                           required
                           value={formData.description}
                           onChange={e => setFormData({...formData, description: e.target.value})}
                           className="w-full bg-white/[0.02] border border-white/10 rounded-2xl p-6 text-sm text-white h-40 resize-none focus:border-primary/50 outline-none transition-all font-medium leading-relaxed shadow-inner"
                           placeholder="Enter payload content for global distribution..."
                         />
                      </div>
                      <div className="space-y-4">
                         <label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 ml-1">Dispatch Matrix</label>
                            ...
                            {['system', 'reward', 'alert'].map(type => (
                               <button
                                 ...
                                 className={cn(
                                    ...
                                    formData.type === type ? "bg-primary border-primary text-white shadow-lg shadow-primary/20" : "bg-white/[0.02] border-white/5 text-white/20 hover:text-white"
                                 )}
                               >
                                  {type}
                               </button>
