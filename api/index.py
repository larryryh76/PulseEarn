...
def send_v():
    u = request.user
    db = get_db()
    user_snap = db.collection('users').document(u['uid']).get()
    user_data = user_snap.to_dict() if user_snap.exists else {}
    username = user_data.get('username', 'Member') if user_data else 'Member'
    ...
    if send_branded_email(u['email'], 'VerifyEmail', {'username': username, 'link': link}, "Verify your PulseEarn account"):
        return jsonify({"success": True})
...
PROVIDERS_ADAPTERS = {
    ...
    'cpagrip': {
        ...
        'identityFields': {
            ...
            'apiKey': {'name': 'User Key / API Key', 'required': True, 'type': 'text', 'placeholder': 'e.g. abc123xyz456'},
            'secret': {'name': 'Secret Key', 'required': True, 'type': 'password', 'placeholder': 'CPAGrip Postback Key'}
        },
        'primaryLaunchField': 'publisherId',
        'launchTemplate': 'https://www.cpagrip.com/show.php?l=0&u={uid}&id={publisherId}'
    }
}
...
OFFERWALL_PROVIDER_REGISTRY = {
    ...
    'cpagrip': {
        'label': 'CPAGrip',
        'user_param': 'user_id', 'tx_param': 'tracking_id', 'offer_param': 'offer_id',
        'offer_name_param': 'offer_title', 'amount_param': 'payout',
        'usd_param': 'payout', 'sig_param': 'signature', 'sig_method': 'none',
        'sig_fields': [],
        'success_response': 'OK',
        'ip_whitelist': [],
    }
...
def _get_cpagrip_offers_cached(pub_id, api_key):
    ...
    if cache_key in _cpagrip_feed_cache:
        return _cpagrip_feed_cache[cache_key][1]
    return []
...
def _normalize_cpagrip_offer(offer, uid, config, host_url):
    ...
    cat = str(offer.get('category', 'other')).lower()
    if 'survey' in cat or 'poll' in cat:
        category = 'surveys'
...
@app.route('/api/offerwall/launch/cpagrip', methods=['GET'])
def cpagrip_launch_offer():
    """Secure CPAGrip Click Flow tracking. Creates the click audit record and redirects to CPAGrip."""
    get_deps()
    if not init_firebase():
        return jsonify({"error": "SERVICE_UNAVAILABLE"}), 503
    db = firestore.client()
    ...
    if not uid or not offer_id:
        return "Missing user or offer identifier.", 400
    ...
    if user_data.get('banned') or user_data.get('status') in ('SUSPENDED', 'BANNED', 'FROZEN'):
        return "Account is under suspension.", 403

    # 2. Get provider config to fetch feed
    p_ref = db.collection('offerwall_providers').document('cpagrip')
    p_snap = p_ref.get()
    if not p_snap.exists or not p_snap.to_dict().get('enabled'):
        return "CPAGrip channel is currently disabled.", 400
    cfg = p_snap.to_dict()
    ...
    db.collection('offer_clicks').document(click_id).set({
        ...
        'status': 'CLICKED',
        'timestamp': firestore.SERVER_TIMESTAMP,
        'offerTitle': matching_offer.get('title'),
        'payout': safe_float(matching_offer.get('payout'))
    })

    # 5. Generate tracking URL (append tracking_id parameter)
    joiner = '&' if '?' in raw_link else '?'
    tracking_url = f"{raw_link}{joiner}tracking_id={click_id}"

    # 6. Redirect to CPAGrip
    from flask import redirect
    return redirect(tracking_url, code=302)


@app.route('/api/offerwall/callback/cpagrip', methods=['GET', 'POST'])
def cpagrip_callback():
    """
    Dedicated CPAGrip postback endpoint.
    Processes conversion events, validates click records, prevents duplicates,
    runs fraud checks, and atomically credits user points via PointTransactionEngine.
    """
    get_deps()
    if not init_firebase():
        return "OK", 200 # Fail-safe ACK
    db = firestore.client()
    ...
    if request.method == 'GET':
        ...
        params = {**dict(request.args), **(request.get_json(force=True, silent=True) or {}), **request.form}
    params = {k: (v[0] if isinstance(v, list) else str(v)) for k, v in params.items()}
    ...
    if user_data.get('banned') or user_data.get('status') in ('SUSPENDED', 'BANNED', 'FROZEN'):
        fraud_flags.append('USER_BANNED')
    if user_data.get('isFlagged') or safe_float(user_data.get('riskScore')) >= 80:
        fraud_flags.append('USER_FLAGGED')
    ...
    # 7. Points Calculation (Authoritative $1 USD = 1000 Points economy base)
    gross_points = payout * OFFERWALL_POINTS_PER_USD * multiplier
    total_pts = min(max(abs(round(gross_points)), min_reward), max_reward)
    user_points = round(total_pts * user_share)
    platform_points = round(total_pts * platform_share)

    is_reversal = (status_str.lower() in ('reversed', 'chargeback', 'refunded', 'cancelled'))
    if is_reversal:
        user_points = -abs(user_points)
        platform_points = -abs(platform_points)
    ...
    def process_cpagrip_reward(txn):
        ...
        post_ledger(txn, user_ref, user_id,
                    tx_type=ledger_tx_type,
                    amount=user_points,
                    xp=(0 if is_reversal else xp_reward),
        ...
        task_hist_ref = user_ref.collection('task_history').document()
        txn.set(task_hist_ref, {
            'claimId': claim_id,
            'taskType': 'offerwall',
            'providerId': 'cpagrip',
...
def calculate_marketplace_operational_intelligence(timeframe='today'):
    ...
    if tf in ('week', '7d'):
        window_cutoff = cutoff_7d
    elif tf in ('month', '30d'):
        window_cutoff = cutoff_30d
    elif tf == 'all':
        window_cutoff = 0
    else:
    ...
    for p_doc in provider_snaps:
        ...
        elif status_raw in ('invalid_credentials', 'callback_failure'):
            conn_status = 'offline'
        else:
            conn_status = status_raw
        ...
        provider_metrics.append({
            ...
            "uptimePercentage": 99.9 if conn_status == 'connected' else (80.0 if conn_status == 'degraded' else 0.0),
    ...
    for t_doc in tasks_snap:
        ...
        category_task_counts[cat] = category_task_counts.get(cat, 0) + 1
        category_reward_dist[cat] = round(category_reward_dist.get(cat, 0.0) + reward, 2)
        total_points_offered += reward

        provider_name = t_data.get('providerName', 'Internal')
        provider_opp_counts[provider_name] = provider_opp_counts.get(provider_name, 0) + 1
        ...
        if reward <= 0:
            integrity_issues.append({
                ...
                "entityId": t_id,
                "entityTitle": title,
                "description": f"Task '{title}' has invalid reward points ({reward}). Requires manual review.",
                "autoFixable": False,
                "detectedAt": now_iso
            })
    ...
    # 3. ECONOMY & USER BEHAVIOR MONITORING (WINDOWED BY TIMEFRAME)
    rewards_snap = db.collection('offerwall_rewards').limit(500).get()
    windowed_rewards = []
    active_earners_24h_set = set()
    returning_earners_7d_set = set()

    for r_doc in rewards_snap:
        r_data = r_doc.to_dict() or {}
        r_ts = 0
        created_at = r_data.get('createdAt') or r_data.get('timestamp')
        if created_at:
            if hasattr(created_at, 'timestamp'):
                r_ts = created_at.timestamp()
            elif isinstance(created_at, str):
                try:
                    r_ts = datetime.fromisoformat(created_at.replace('Z', '')).timestamp()
                except Exception:
                    pass

        uid = r_data.get('userId')
        if uid:
            if r_ts >= cutoff_24h or cutoff_24h <= 0:
                active_earners_24h_set.add(uid)
            if r_ts >= cutoff_7d or cutoff_7d <= 0:
                returning_earners_7d_set.add(uid)

        if r_ts >= window_cutoff or window_cutoff == 0:
            windowed_rewards.append(r_data)

    total_rewards_issued_pts = sum([safe_float(r.get('points', 0)) for r in windowed_rewards])
    ...
    active_earners_24h = len(active_earners_24h_set) or 1
    returning_earners_7d = len(returning_earners_7d_set) or max(1, int(active_earners_24h * 0.65))
    ...
    report = {
        ...
        "economy": {
            ...
            "outstandingRewardsCount": len(windowed_rewards),
        ...
        "activeAlerts": alerts,
        "_categoryTaskCounts": category_task_counts,
        "_providerOppCounts": provider_opp_counts
    }
...
def get_admin_marketplace_stats():
    ...
    try:
        ...
        category_counts = intel.get('_categoryTaskCounts', {})
        category_breakdown = []
        ...
        provider_counts = intel.get('_providerOppCounts', {})
        provider_health = []
        for p in intel['providers']:
            p_name = p['providerName']
            provider_health.append({
                "id": p['providerId'],
                "name": p_name,
                "status": p['connectionStatus'] if p['connectionStatus'] in ('connected', 'degraded', 'offline', 'disabled') else 'offline',
                "opportunities": provider_counts.get(p_name, provider_counts.get(p_name.lower(), 0)),
...
def run_marketplace_integrity_audit():
    ...
    auto_repair = body.get('autoRepair', False)
    ...
    for t_doc in tasks_snap:
        ...
        if reward <= 0:
            issue = {
                ...
                "entityId": t_id,
                "entityTitle": title,
                "description": f"Task '{title}' has invalid reward ({reward}). Requires manual review.",
                "autoFixable": False,
                "detectedAt": now_iso
            }
            issues_found.append(issue)
...
def get_current_bnb_gbp_price():
    ...
    try:
        rate, _ = fetch_psemine_bnb_gbp_quote()
        if rate and rate > 0:
            return float(rate)
    except Exception as e:
        logging.warning(f"[PSEmine] Quote fetch error: {e}")
    return None
...
def generate_psemine_tool_quote():
    ...
    if not exchange_rate or exchange_rate <= 0:
        return jsonify({
            "success": False,
            "error": "PRICE_SERVICE_UNAVAILABLE",
            "message": "Live BNB/GBP exchange rate is currently unavailable. Please try again in a moment."
        }), 503
    ...
    # Persist quote server-side by quoteId for authoritative purchase verification
    try:
...
def verify_psemine_tool_purchase():
    ...
    Validates transaction hash format, checks for replay attacks, verifies receipt recipient and payable
    value against quoted BNB amount on BNB Smart Chain, and commits the ownership record, purchase status,
    user capacity, and campaign aggregate counters inside an atomic Firestore transaction.
    """
    db = get_db()
    if not db: return jsonify({"success": False, "error": "SERVICE_UNAVAILABLE"}), 503
    uid = request.user['uid']
    data = request.get_json() or {}
    purchase_id = (data.get('purchaseId') or '').strip()
    tx_hash = (data.get('transactionHash') or '').strip().lower()
    sender_wallet = (data.get('senderWallet') or '').strip().lower()

    if not purchase_id:
        return jsonify({"success": False, "error": "MISSING_PURCHASE_ID"}), 400

    if not tx_hash or not tx_hash.startswith('0x') or len(tx_hash) != 66:
        return jsonify({"success": False, "error": "INVALID_TRANSACTION_HASH", "message": "Transaction hash must be a 66-character hex string starting with 0x."}), 400

    # 1. Fetch purchase intent
    purchase_ref = db.collection('psemine_purchases').document(purchase_id)
    purchase_snap = purchase_ref.get()
    if not purchase_snap.exists:
        return jsonify({"success": False, "error": "PURCHASE_NOT_FOUND", "message": "Purchase intent record not found."}), 404

    purchase = purchase_snap.to_dict() or {}
    if purchase.get('userId') != uid:
        return jsonify({"success": False, "error": "FORBIDDEN", "message": "This purchase does not belong to your account."}), 403
    ...
    if expires_at_raw:
        try:
            ...
            if exp_dt.tzinfo is None:
                exp_dt = exp_dt.replace(tzinfo=timezone.utc)
            if datetime.now(timezone.utc) > exp_dt:
                return jsonify({"success": False, "error": "QUOTE_EXPIRED", "message": "Quote has expired. Please create a new purchase."}), 409
        except Exception:
            pass

    # 3. Prevent replay attacks: check prior redemption before attempting verification
    dupes_p = db.collection('psemine_purchases').where('transactionHash', '==', tx_hash).where('status', '==', 'activated').get()
    if len(dupes_p) > 0:
        return jsonify({"success": False, "error": "TRANSACTION_ALREADY_REDEEMED", "message": "Transaction hash has already been redeemed."}), 409

    dupes_pay = db.collection('psemine_payments').where('txHash', '==', tx_hash).get()
    if len(dupes_pay) > 0:
        return jsonify({"success": False, "error": "TRANSACTION_ALREADY_REDEEMED", "message": "Transaction hash has already been redeemed."}), 409

    dupe_redeemed = db.collection('psemine_redeemed_hashes').document(tx_hash).get()
    if dupe_redeemed.exists:
        return jsonify({"success": False, "error": "TRANSACTION_ALREADY_REDEEMED", "message": "Transaction hash has already been redeemed."}), 409

    # 4. Verify tool definitions & tier limits
    tool_id = purchase.get('toolId')
    if tool_id not in LOCKED_PSEMINE_TOOLS_CONFIG:
        return jsonify({"success": False, "error": "INVALID_TOOL_TIER"}), 400
    ...
    user_snap = user_ref.get()
    u_data = user_snap.to_dict() if user_snap.exists else {}

    current_owned = (u_data.get('toolOwnershipCounts') or {}).get(tool_id, 0)
    if current_owned >= tool_cfg['max_per_user']:
        return jsonify({
            "success": False, 
            "error": "MAX_OWNERSHIP_REACHED", 
    ...
    verified, err_code, err_msg, block_depth, receipt_block = verify_bsc_transaction(
        ...
        expected_wei,
        expected_sender=expected_sender if expected_sender else None
    )

    if not verified:
        allow_skip = (
            os.environ.get('PSEMINE_SKIP_ONCHAIN_CHECK') == 'true'
        ...
        logging.warning(f"[PSEmine] Non-production on-chain check bypassed for {tx_hash}: {err_code}")

    # 6. Atomic Firestore Transaction Execution
    ownership_id = f"own_{tool_id}_{uid[:5]}_{int(time.time() * 1000)}"
    ...
    prev_tool_counts = u_data.get('toolOwnershipCounts') or {}
    is_first_tool_activation = sum(prev_tool_counts.values()) == 0
    ...
    try:
        ...
        def commit_purchase_activation(txn):
            ...
            prev_accrued = float(u_dict.get('totalAccruedGBP', 0.0))
            last_accrued_str = u_dict.get('lastAccruedAt')
            new_accrued = prev_accrued
            if last_accrued_str and prev_total_cap > 0:
                try:
                    last_dt = datetime.fromisoformat(last_accrued_str.replace('Z', '+00:00'))
                    if last_dt.tzinfo is None:
                        last_dt = last_dt.replace(tzinfo=timezone.utc)
                    elapsed_sec = max(0, (now_dt - last_dt).total_seconds())
                    new_accrued = prev_accrued + (elapsed_sec / 3600.0) * prev_total_cap
                except Exception:
                    pass

            # Record redeemed hash atomically
            txn.set(redeemed_ref, {
                'txHash': tx_hash,
            ...
            if c_snap.exists:
                txn.update(camp_ref, {
                    ...
                    'totalBNBCollected': firestore.Increment(quoted_bnb),
                    'totalMinersCount': firestore.Increment(1 if user_was_inactive else 0),
                    'updatedAt': now_iso,
                })
            else:
                txn.set(camp_ref, {
        ...
        return jsonify({
            ...
            "transactionHash": tx_hash,
            "confirmedAt": now_iso,
            "message": f"Transaction verified and {tool_cfg['name']} deployed successfully."
        })

    except Exception as e:
        err_str = str(e)
        if err_str == "TRANSACTION_ALREADY_REDEEMED":
            return jsonify({"success": False, "error": "TRANSACTION_ALREADY_REDEEMED", "message": "Transaction hash has already been redeemed."}), 409
        if err_str == "PURCHASE_ALREADY_ACTIVATED":
            return jsonify({"success": False, "error": "PURCHASE_ALREADY_ACTIVATED", "message": "This purchase has already been activated."}), 409
        logging.exception(f"[PSEmine Purchase Verify Error]: {e}")
        return jsonify({
            "success": False, 
            "error": "TRANSACTION_FAILED", 
            "message": "Activation could not be completed. Contact support with your transaction hash."
        }), 500
