...
export const PSEmineApp: React.FC = () => {
  ...
  return <div className="psemine-app-page">
    ...
    <main className="psemine-shell psemine-app-main">
      {location.pathname !== '/mine/dashboard' && <section className="psemine-app-section psemine-feature-state"><span className="psemine-eyebrow">{navItems.find(item => item.href === location.pathname)?.label || 'PSEmine'}</span><h2>This workspace is ready for authoritative data.</h2><p>That capability is not enabled in the current backend configuration. Nothing has been simulated or mutated in the browser.</p><Link to="/mine/dashboard" className="psemine-button psemine-button-secondary">Return home <ArrowRight /></Link></section>}
