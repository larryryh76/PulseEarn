  ...
  return (
    <div className="space-y-12">
       <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          ...
          <div className="flex items-center gap-3">
             <button
               ...
               className="px-8 py-3 bg-primary text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-primary/90 transition-all flex items-center gap-2 shadow-lg shadow-primary/20"
             >
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-[#0A0A0F] border border-white/5 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
             <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 mb-4">Point Supply (Sample)</p>
          ...
          <div className="bg-[#0A0A0F] border border-white/5 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
             <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 mb-4">Total USD Liability</p>
          ...
          <div className="bg-[#0A0A0F] border border-white/5 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
             <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 mb-4">Market Liability</p>
          ...
          <div className="bg-[#0A0A0F] border border-white/5 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
             <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 mb-4">XP Provisioned</p>
       ...
       <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          <div className="xl:col-span-2 space-y-8">
             <section className="bg-[#0A0A0F] border border-white/5 rounded-[2.5rem] p-10 shadow-2xl">
                <h2 className="text-sm font-black uppercase tracking-[0.3em] text-white/20 flex items-center gap-3 mb-10">
                   <Activity size={18} className="text-primary" /> Real-time Feed
                </h2>
                <div className="space-y-1">
                   {recentTransactions.map(tx => (
                      <div key={tx.id} className="flex items-center justify-between p-5 rounded-2xl bg-white/[0.01] border-b border-white/5 last:border-0 hover:bg-white/[0.03] transition-all">
                         <div className="flex items-center gap-5">
                            <div className={tx.amount > 0 ? "text-success bg-success/5 p-3 rounded-xl" : "text-white bg-white/5 p-3 rounded-xl"}>
                               {tx.amount > 0 ? <ArrowUpRight size={18} /> : <ArrowDownLeft size={18} />}
                            </div>
                            <div>
                               <p className="text-xs font-bold text-white uppercase italic">{tx.source}</p>
             ...
             <section className="bg-[#0A0A0F] border border-white/5 rounded-[2.5rem] p-10 shadow-2xl">
                ...
                <div className="space-y-4">
                   {[
                     ...
                     <div key={item.label} className="flex items-center justify-between p-5 rounded-2xl bg-white/[0.01] border border-white/5 group hover:border-primary/20 transition-all">
                        <p className="text-[10px] font-black text-text-tertiary uppercase tracking-widest">{item.label}</p>
                        <p className="text-xs font-mono font-bold text-white group-hover:text-primary transition-all">{item.value}</p>
                     </div>
                   ))}
                </div>
             </section>
          ...
          {isAdjusting && (
             <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                <motion.div
                  ...
                  className="relative w-full max-w-lg bg-[#0A0A0F] border border-white/10 rounded-[3rem] shadow-[0_0_80px_rgba(0,0,0,1)] overflow-hidden flex flex-col"
                >
                   <div className="p-10 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
                      <div className="flex items-center gap-6">
                         ...
                         <div>
                            <h3 className="text-2xl font-bold text-white tracking-tight uppercase italic leading-none mb-2">Economy Mutation</h3>
                            <p className="text-white/40 text-[10px] font-black uppercase tracking-widest leading-none">Authorized Admin Logic Vector</p>
                         </div>
                      </div>
                   ...
                   <div className="p-10 space-y-10">
                      <div className="space-y-3">
                         <label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 ml-1 flex items-center gap-2">
                         ...
                         <input
                           ...
                           className="w-full bg-white/[0.02] border border-white/10 rounded-2xl px-6 py-5 text-sm text-white focus:border-primary/50 outline-none transition-all font-mono"
                         />
                      </div>

                      <div className="grid grid-cols-2 gap-8">
                         <div className="space-y-3">
                            <label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 ml-1">Asset Delta</label>
                            <div className="relative group">
                               ...
                               <div className="absolute right-4 top-1/2 -translate-y-1/2 flex gap-1.5">
                                  <button onClick={() => setAdjustForm(prev => ({ ...prev, amount: Math.abs(prev.amount) }))} className={cn("p-1.5 rounded-lg transition-all", adjustForm.amount >= 0 ? "bg-success/20 text-success" : "bg-white/5 text-white/20")}><Plus size={14} /></button>
                                  <button onClick={() => setAdjustForm(prev => ({ ...prev, amount: -Math.abs(prev.amount) }))} className={cn("p-1.5 rounded-lg transition-all", adjustForm.amount < 0 ? "bg-danger/20 text-danger" : "bg-white/5 text-white/20")}><Minus size={14} /></button>
                               </div>
                            </div>
                         </div>
                         <div className="space-y-3">
                            <label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 ml-1">Asset Matrix</label>
                            <select
                              value={adjustForm.isXp ? 'XP' : 'POINTS'}
                              onChange={e => setAdjustForm(prev => ({ ...prev, isXp: e.target.value === 'XP' }))}
                              className="w-full bg-white/[0.02] border border-white/10 rounded-2xl px-6 py-5 text-sm text-white focus:border-primary/50 outline-none transition-all font-black uppercase tracking-widest appearance-none"
                            >
                      ...
                      <div className="space-y-3">
                         <label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 ml-1">Operational Rationale</label>
                         <textarea
                           rows={3}
                           ...
                           className="w-full bg-white/[0.02] border border-white/10 rounded-2xl px-6 py-5 text-sm text-white h-32 resize-none focus:border-primary/50 outline-none transition-all font-medium leading-relaxed"
                         />
                      </div>
                      ...
                      <div className="pt-4 flex gap-4">
                         ...
                         <button
                           type="button"
                           onClick={() => setIsAdjusting(false)}
                           className="px-10 py-6 rounded-2xl bg-white/[0.02] border border-white/10 text-white/20 hover:text-white transition-colors font-black uppercase tracking-widest text-[10px]"
                         >
                            Abort
                         </button>
                      </div>
