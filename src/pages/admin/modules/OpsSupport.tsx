...
const OpsSupport: React.FC = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'CLOSED': return 'text-white/20 border-white/5 bg-white/5';
  ...
  return (
    <div className="space-y-12">
       <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="space-y-2">
             <div className="flex items-center gap-3">
                ...
                <h1 className="text-3xl font-bold tracking-tight uppercase italic text-white">Support Desk</h1>
             </div>
          ...
          <div className="flex gap-4">
             <div className="relative group">
                ...
                <input
                  ...
                  className="bg-white/5 border border-white/10 rounded-xl pl-12 pr-6 py-3 text-sm text-white focus:border-primary/50 w-80"
                />
             </div>
             <select
               value={filter}
       ...
       <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-[800px]">
          <div className="lg:col-span-4 flex flex-col bg-[#0A0A0F] border border-white/5 rounded-3xl overflow-hidden shadow-2xl">
             <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
                <h3 className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">Queue Matrix</h3>
                ...
                {filteredTickets.map((ticket) => (
                   ...
                   >
                      <div className="flex justify-between items-start mb-3">
                         ...
                         <span className="text-[9px] font-mono text-white/10 uppercase tracking-widest">{ticket.updatedAt?.toDate?.()?.toLocaleDateString()}</span>
                      </div>
                      <h4 className="text-sm font-bold text-white uppercase tracking-tight italic truncate group-hover:text-primary transition-colors">{ticket.subject}</h4>
                      ...
                      <p className="text-[10px] text-white/10 mt-3 truncate italic leading-relaxed">{ticket.lastMessagePreview}</p>
                   </div>
                ))}
             </div>
          </div>

          <div className="lg:col-span-8 flex flex-col bg-[#08080C] border border-white/5 rounded-3xl overflow-hidden shadow-2xl relative">
             {selectedTicket ? (
                <>
                   <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.01] z-10">
                      <div className="flex items-center gap-6">
                         <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center text-white/20 shadow-inner">
                            <Mail size={24} />
                         </div>
                         <div>
                            <h3 className="text-xl font-bold text-white uppercase tracking-tighter italic leading-none mb-2">{selectedTicket.subject}</h3>
                            <div className="flex items-center gap-4 text-xs font-medium">
                               ...
                               <span className="text-[10px] font-mono text-white/20 uppercase tracking-widest">ID: {selectedTicket.id}</span>
                            </div>
                         </div>
                      </div>
                      <div className="flex items-center gap-3">
                   ...
                   <div className="flex-1 overflow-y-auto p-10 space-y-12 no-scrollbar bg-black/20 shadow-inner">
                      {messages.map((msg) => (
                         <div key={msg.id} className={cn("flex gap-6 max-w-[85%]", msg.senderType === 'ADMIN' ? "ml-auto flex-row-reverse" : "mr-auto")}>
                            <div className={cn(
                               ...
                               msg.senderType === 'ADMIN' ? "bg-primary border-primary/20 text-white shadow-primary/10" : "bg-white/[0.05] border-white/10 text-white/40"
                            )}>
                            ...
                            <div className="space-y-3">
                               <div className={cn(
                                  ...
                                  msg.senderType === 'ADMIN' ? "bg-primary border-primary/20 text-white" : "bg-white/[0.03] border-white/5 text-text-secondary"
                               )}>
                                  {msg.text}
                                  ...
                                  {msg.attachments && msg.attachments.length > 0 && (
                                     <div className="mt-8 pt-8 border-t border-white/5 flex flex-wrap gap-3">
                                        {msg.attachments.map((at, i) => (
                                           <a key={i} href={at.storageUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-black/40 text-[10px] font-bold text-white/60 hover:text-white transition-all border border-white/5 shadow-inner">
                                              <Paperclip size={14} />
                                              {at.fileName}
                                           </a>
                                        ))}
                               ...
                               <div className={cn("flex items-center gap-3 px-4", msg.senderType === 'ADMIN' ? "justify-end" : "justify-start")}>
                                  <span className="text-[10px] font-black uppercase tracking-widest text-white/10">{msg.senderName}</span>
                   ...
                   <div className="p-8 bg-black/40 border-t border-white/5 space-y-6">
                      <div className="relative group">
                         <textarea
                           ...
                           className="w-full bg-white/[0.02] border border-white/10 rounded-[2rem] px-8 py-6 text-sm text-white focus:border-primary/50 outline-none transition-all font-medium resize-none shadow-2xl"
                         />
                         <div className="absolute right-6 bottom-6 flex items-center gap-4">
                            <button className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center text-white/10 hover:text-white transition-all border border-white/5">
                               <Paperclip size={20} />
                            </button>
                            <Button
                ...
                <div className="flex-1 flex flex-col items-center justify-center space-y-8 opacity-20">
                   ...
                   <div className="text-center space-y-3">
                      <h3 className="text-2xl font-bold text-white uppercase tracking-widest italic">Select Node</h3>
                      <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.6em]">System Ingress Pending Analysis</p>
                   </div>
                </div>
             )}
          </div>
